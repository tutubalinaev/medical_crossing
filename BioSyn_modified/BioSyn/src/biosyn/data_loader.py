...
class QueryDataset(Dataset):
    ...
    def load_data(self, data_dir, filter_composite, filter_duplicate, use_context=False):
        """       
        Parameters
        ----------
        data_dir : str
        ...
        for concept_file in tqdm(concept_files, f"reading concept files from {data_dir}"):
            ...
            for concept in concepts:
                ...
                if use_context:
                    mention = concept[5] + " [unused1] " + concept[3].strip() + " [unused1] " + concept[6]
                else:
...
class CandidateDataset(Dataset):
    ...
    """
    def __init__(self, queries, dicts, tokenizer, topk, d_ratio,
                 #s_score_matrix,
                 dict_sparse_embeds,
                 query_sparse_embeds,
                 s_candidate_idxs, max_length):
        """
        ...
        self.query_sparse_embeds = query_sparse_embeds
    ...
    def __getitem__(self, query_idx):
        ...
        query_token = self.tokenizer(query_name, return_tensors="pt", padding='max_length', truncation=True,
                                                       max_length=self.max_length).to(torch.device("cuda"))
        ...
        candidate_s_scores = np.matmul(self.query_sparse_embeds[query_idx], self.dict_sparse_embeds[topk_candidate_idx].T)
        #if scipy.sparse.issparse(self.s_score_matrix):
        #    candidate_s_scores = self.s_score_matrix[query_idx, topk_candidate_idx].toarray()
        #else:
        ...
        candidate_tokens = self.tokenizer(candidate_names, return_tensors="pt", padding='max_length', truncation=True,
                                                       max_length=self.max_length).to(torch.device("cuda"))
