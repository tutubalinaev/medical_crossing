...
class RerankNet(nn.Module):
    ...
    def forward(self, x):
        ...
        query_token = {k: v.squeeze(dim=1) for k, v in query_token.items()}
        candidate_tokens = {k: v.squeeze(dim=1) for k, v in candidate_tokens.items()}

        batch_size, topk, _ = candidate_tokens['input_ids'].shape
        if self.use_cuda:
            candidate_s_scores = candidate_s_scores.cuda()

        # dense embed for query and candidates
        query_embed = self.encoder(**query_token)[0]
        query_embed = query_embed[:, 0].unsqueeze(dim=1)

        candidate_masks = self.reshape_candidates_for_encoder(candidate_tokens['attention_mask'])
        candidate_tokens = self.reshape_candidates_for_encoder(candidate_tokens['input_ids'])

        candidate_embeds = self.encoder(candidate_tokens, attention_mask=candidate_masks)[0]
        candidate_embeds = candidate_embeds[:, 0].view(batch_size, topk, -1)
