...
import pandas as pd
from biosyn.scispacy_abbr_resolver import SciSpacyAbbrResolver
...
def parse_concept(path):
    ...
    with open(path, 'r') as f:
        for line in f:
            line = line.strip()
            pmid, span, mention_type, mention, cui, left_ctx, right_ctx = line.split('||')
            result.append([pmid, span, mention_type, mention, cui, left_ctx, right_ctx])
    return result
...
def apply_basic_preprocess(concept, text_preprocessor):
    result = []
    for pmid, span, mention_type, mention, cui, left_ctx, right_ctx in concept:
        result_mention = text_preprocessor.run(mention)
        result.append([pmid, span, mention_type, result_mention, cui, left_ctx, right_ctx])
    return result
...
def apply_composite_resolve(concept):
    ...
    for pmid, span, mention_type, mention, cui, left_ctx, right_ctx in concept:
        ...
        if len(cui_list) > 1 and len(mention_list) == len(cui_list):
            result.append([pmid, span, mention_type, '|'.join(mention_list), composite_symbol.join(cui_list)], left_ctx, right_ctx)
        else:
            result.append([pmid, span, mention_type, mention, cui, left_ctx, right_ctx])
    return result
...
def parse_args():
    ...
    # Text Preprocessing
    parser.add_argument('--ab3p_path',  type=str, default="../Ab3P/identify_abbr", help='abbreviation resolution module')
...
def main(args):
    ...
    # abbr_resolver = SciSpacyAbbrResolver()
