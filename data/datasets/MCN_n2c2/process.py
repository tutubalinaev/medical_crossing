...
def save_annotations(annotations, save_to):
    with open(save_to, "w+") as fout:
        for ann in annotations:
            ...
            # the last two are required as 'left/right' contexts by BioSyn
            fout.write('{}||{}|{}||Entity||{}||{}||||\n'.format(ann['id'],
                                                            start_pos, end_pos,
                                                            entity, ann['cui']))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--file_list', default='train/train_file_list.txt')
    parser.add_argument('--annotation_paths', default='train/train_norm/')

    # chunks of texts containing the entities
    # parser.add_argument('--note_paths', default='test/test_note/')
    # parser.add_argument('--save_to', default='processed_pairs/')
    parser.add_argument('--note_paths', default='train/train_note/')
    parser.add_argument('--save_to', default='processed_pairs/train/')
    ...
    for annotation_path, note_path in tqdm(zip(annotation_paths, note_paths)):
